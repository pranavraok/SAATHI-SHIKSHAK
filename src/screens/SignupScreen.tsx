...
export function SignupScreen({ lang, onNext, onBack }: { lang: Lang; onNext: (s: Screen) => void; onBack: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center justify-between min-h-screen px-6 py-8"
          ...
          <div className="flex items-center rounded-2xl overflow-hidden shadow-sm"
            ...
            <input type="text" value={name}
              ...
              style={{
                ...
                fontSize: 16, color: C.slate900, background: "transparent",
              }} />
          </div>
        </div>
      ...
      <div className="w-full max-w-sm flex flex-col mt-auto mb-4">
        <button onClick={() => name.length > 0 && phone.length === 10 && onNext("otp")}
          className="w-full rounded-full transition-all active:scale-95"
          style={{
            padding: "16px 0", minHeight: 52,
            background: name.length > 0 && phone.length === 10 ? C.primary : C.slate300,
            color: C.white, fontSize: 17, fontWeight: 600, border: "none",
            cursor: name.length > 0 && phone.length === 10 ? "pointer" : "default",
            boxShadow: name.length > 0 && phone.length === 10 ? `0 4px 16px ${C.primary}40` : "none",
          }}>
          {t.createAccount}
        </button>
      </div>
